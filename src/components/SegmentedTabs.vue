<template>
  <TabList
    ...
    class="transaction-tabs bg-surface-secondary text-ui relative z-0 flex w-full flex-row items-center rounded-xl"
  >
    <Tab
    ...
    >
      <button
        ...
        class="transaction-tab flex flex-1 flex-row items-center justify-center gap-1 outline-none"
        type="button"
        :data-sync="'tab-' + option.id"
      >
        <component :is="option.icon" v-if="option.icon" :size="16" />
        {{ option.label }}
        <span v-if="option.count !== undefined && option.count >= 0">({{ option.count }})</span>
      </button>
    </Tab>
...
button.transaction-tab {
  ...
  @apply h-10 w-full cursor-pointer px-2 py-1;
}
