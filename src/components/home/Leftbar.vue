<template>
  <div class="left-sticky-bar">
    ...
    <div>
      <!-- DP -->
      <div class="leftbar-headshot relative h-16 w-16 rounded-full bg-blue-600 select-none">
        <div
      ...
      <h1 class="leftbar-headshot mt-8 mb-4 text-4xl font-bold">Hi, I’m Karan</h1>
      <p class="leftbar-headshot text-base font-medium">
        I’m still searching for a one-liner to sum me up, but until then my life is a bento box of
        endless interests, neatly packed for display on my ever-evolving personal website.
      </p>
    </div>
    ...
    <div class="flex flex-col gap-2 overflow-visible">
      ...
      <div class="flex w-full flex-row flex-wrap gap-2 overflow-visible">
        <Pill id="email" text="hi@psiderman.com" icon="envelope" @click.prevent="copyEmail" />
        <!-- <Pill
          text="Twitter"
          brand="twitter"
          link="https://twitter.com/_psiderman_"
        /> -->
        <Pill text="Instagram" brand="instagram" link="https://instagram.com/psiderman" />
        <Pill
          text="Reddit"
          brand="reddit-alien"
          link="https://www.youtube.com/watch?v=xvFZjo5PgG0"
        />
        <Pill text="LinkedIn" brand="linkedin-in" link="https://www.linkedin.com/in/psiderman/" />
      </div>
    </div>
  </div>
</template>
  ...
  navigator.clipboard
    ...
    .then(() => {
      ...
      if (event.x == 0 && event.y == 0) {
        messageX = emailRect.x + emailRect.width / 2 - copiedMessage.offsetWidth / 2
        messageY =
          emailRect.y + emailRect.height / 2 - copiedMessage.offsetHeight / 2 - 20 + scrollYOffset
      } else {
        messageX = event.clientX - copiedMessage.offsetWidth / 2
        messageY = event.clientY - copiedMessage.offsetHeight / 2 - 20 + scrollYOffset
      }
