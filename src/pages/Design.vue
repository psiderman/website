<template>
  <div class="mx-auto min-h-dvh w-screen max-w-screen-xl px-0 py-20">
    ...
    >
      <h1 class="anime-entry text-center text-4xl font-semibold text-white">Design Portfolio</h1>
      <p class="anime-entry text-center text-base text-white/80">
        Most recently, I was a senior product designer at Dezerv (Accel-backed, $2Bn AUM) where I
        led the first ever desktop investor experience and introduced Prosper, the design system
        that unifies design foundations across 3 investor products and 1 internal tool.
      </p>
    </div>
      ...
      <div v-for="(col, i) in gridItems" :key="i" class="flex flex-col gap-4">
        <template v-for="(shot, j) in col" :key="j">
          <DesignCard
            :title="shot.title"
            :year="shot.year"
            :class="[shot.aspect ? shot.aspect : '', shot.link ? 'cursor-pointer' : '']"
          ...
          >
            ...
            <img
              ...
              :alt="shot.title"
            />
            <component :is="shot.component" v-else-if="shot.type == 'component'"></component>
            <img
              v-if="shot.link"
    ...
    >
      <span class="footer-anime-entry"> ©️ Karan Sanas {{ new Date().getFullYear() }} </span>
    ...
    <div class="w640 mt-8 flex flex-row flex-wrap justify-center text-white/50 select-none">
      <RouterLink
        to="/"
...
const shotData = [
  ...
  {
    link: false,
    src: new URL('@/assets/design/shots/button.mp4', import.meta.url).toString(),
  ...
  },
  {
    aspect: 'aspect-shot',
    link: false,
    src: new URL('@/assets/design/shots/graphing.mp4', import.meta.url).toString(),
  ...
  },
  {
    ...
    src: new URL('@/assets/design/shots/owensans.mp4', import.meta.url).toString(),
    title: 'Owen Sans',
    type: 'video',
    year: 'Feb 2023',
  },
  {
    link: false,
    src: new URL('@/assets/design/shots/graphing.png', import.meta.url).toString(),
  ...
  },
  {
    ...
    src: new URL('@/assets/design/shots/primer-2.mp4', import.meta.url).toString(),
  ...
  {
    link: false,
    src: new URL('@/assets/design/shots/psiderman.com.mp4', import.meta.url).toString(),
    title: 'Old Portfolio',
    type: 'video',
    year: '2022',
  },
...
function transformData(cols) {
  ...
  for (let i = 0; i < shotData.length; i++) gridItems.value[i % gridCols.value].push(shotData[i])
}
