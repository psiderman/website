<script setup>
import Pill from "@/components/Pill.vue";
</script>

<template>
  <!-- Content -->
  <div
    class="mx-auto flex h-screen max-h-dvh w-screen max-w-screen-lg flex-row gap-10 overflow-scroll py-20"
  >
    <!-- Left bar -->
    <div
      class="sticky top-0 flex h-full w-80 shrink-0 flex-col justify-between"
    >
      <!-- Quick intro -->
      <div class="flex flex-col gap-y-8">
        <div class="relative h-16 w-16 rounded-full bg-blue-600">
          <div
            class="absolute bottom-0 h-20 w-16 overflow-hidden rounded-b-full align-bottom"
          >
            <img
              class="h-20 w-20 object-cover"
              src="@/assets/images/dp.png"
              alt=""
            />
          </div>
        </div>
        <div class="flex flex-col gap-y-4">
          <h1 class="text-4xl font-bold">Hi, I’m Karan</h1>
          <p class="text-base font-medium">
            I’m still searching for a one-liner to sum me up, but until then my
            life is a bento box of endless interests, neatly packed for display
            on my ever-evolving personal website.
          </p>
        </div>
      </div>

      <!-- Socials/External Links -->
      <div class="flex w-full flex-row flex-wrap gap-2">
        <Pill text="hi@psiderman.com">
          <template v-slot:icon>
            <img src="@/assets/duotone/envelope.svg" />
          </template>
        </Pill>
        <a href="https://twitter.com/_psiderman_" target="_blank">
          <Pill text="Twitter" brand="twitter" />
        </a>
        <a href="https://dribbble.com/psiderman" target="_blank">
          <Pill text="Dribbble" brand="dribbble" />
        </a>
        <a href="https://www.youtube.com/watch?v=xvFZjo5PgG0" target="_blank">
          <Pill text="Reddit" brand="reddit-alien" />
        </a>
      </div>
    </div>

    <!-- Right bar -->
    <div class="grid min-h-fit w-full grid-cols-2 gap-10">
      <div class="card card-big"></div>
      <div class="card card-big"></div>
      <div class="card card-big"></div>
      <div class="grid grid-cols-2 gap-10">
        <div class="card card-small"></div>
        <div class="card card-small"></div>
        <div class="card card-small"></div>
      </div>
      <div class="col-span-2 h-10"></div>
    </div>
  </div>
</template>

<style lang="postcss" scoped>
.card {
  @apply rounded-3xl border border-white/5 bg-white/[2%];
}

.card-big {
  @apply h-80;
}

.card-small {
  @apply h-[140px];
}
</style>
